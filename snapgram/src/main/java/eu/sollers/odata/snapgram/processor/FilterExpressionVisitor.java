...
public class FilterExpressionVisitor implements ExpressionVisitor<String> {
    ...
    private JpaEntityCsdlProvider jpaProvider;

    public FilterExpressionVisitor(JpaEntityCsdlProvider csdlProvider) {
        this.entityAlias = csdlProvider.getBackingClass().getSimpleName().toLowerCase();
        this.jpaProvider = csdlProvider;
    }
    ...
    public String visitMember(Member member) throws ExpressionVisitException, ODataApplicationException {
        ...
        if (resources.size() == 1 && first instanceof UriResourcePrimitiveProperty) {
            ...
            return entityAlias + "." + jpaProvider
                    .getJavaPropertyForODataProperty(primitiveProperty.getProperty().getName());
        } else {
